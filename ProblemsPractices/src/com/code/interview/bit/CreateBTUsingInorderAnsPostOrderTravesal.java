...
public class CreateBTUsingInorderAnsPostOrderTravesal {
	class TreeNode {
		int val;
		TreeNode left;
		TreeNode right;
	...
	public TreeNode buildTree(ArrayList<Integer> inorder, ArrayList<Integer> postorder) {
		...
		TreeNode root = createBT(inorder,postorder,map,0,inorder.size()-1,0,postorder.size()-1); 
		return root;
	}
	private TreeNode createBT(ArrayList<Integer> inorder,
		...
		if(il<ir) {
			...
			int index = map.get(postorder.get(or));
			int oIndex = index-il-1;
			node.left = createBT(inorder, postorder, map, il, index-1, ol, ol+oIndex);
			node.right = createBT(inorder, postorder, map, index+1, ir, ol+oIndex+1, or-1);
			return node;
